...
var DateTimePickerExtension = (function () {
    ...
    return {
        ...
        setDate: function (instanceId, date) {
            ...
            if (e) {
                e.data('datetimepicker').date(date !== null ? moment(date, 'DD/MM/yyyy HH:mm') : date);
            }
        },
        getDate: function (instanceId) {
            ...
            if (e) {
                try {
                    let dateString = moment(e.datetimepicker("date")).format('DD/MM/yyyy HH:mm');
...
function EnableSortable(id, handle) {
    $(id).sortable({placeholder:'sort-highlight',connectWith:'.connectedSortable',handle: handle,forcePlaceholderSize:true,zIndex:999999})
    $(`${id} .card-header`).css('cursor','move')
}

function DisableSortable(id) {
    try {
        $(id).sortable('destroy')
        $(`${id} .card-header`).css('cursor', '')
    }
    catch (error) { 
        console.log("loi")
    }
...
// ChartJSExtension
var ChartJSExtension = (function () {
    let chartInstances = {};

    function init(canvasId, instanceId, options) {
        let element = document.getElementById(canvasId);
        if (element == null) return;
        let ctx = element.getContext('2d');
        let chartInstance = new Chart(ctx, options);
        chartInstances[instanceId] = chartInstance;
        return chartInstance;
    }

    return {
        init: init,
        update: function (instanceId, labels, data) {
            let instance = chartInstances[instanceId];
            if (instance) {
        ...
        destroy: function (instanceId) {
            ...
            if (instance) {
                instance.destroy();
                delete chartInstances[instanceId];
            }
        }
    };
})();
...
function renderFeatureQuotaRow(featureQuotaRowElement) {
    const featureQuotasTableBody = document.getElementById('featureQuotasTableBody');
    featureQuotasTableBody.innerHTML = ''; // Xóa nội dung hiện có trong phần tử featureQuotasTableBody
    featureQuotasTableBody.insertAdjacentHTML('beforeend', featureQuotaRowElement);
}
